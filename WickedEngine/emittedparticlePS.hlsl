...
float4 main(VertextoPixel PSIn) : SV_TARGET
{
	...
	float fade = saturate(1.0 / PSIn.opaSiz.y*(max(max(depthScene.x, depthScene.y), max(depthScene.z, depthScene.w)) - depthFragment));

	color.a = saturate(color.a * PSIn.opaSiz.x * fade);
