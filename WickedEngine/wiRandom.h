...
namespace wi::random
{
	...
	struct RNG
	{
		...
		constexpr uint64_t next_uint(uint64_t min, uint64_t max)
		{
			if (min == max)
				return min;
			return min + (next_uint() % (std::min(uint64_t(0xffffffffffffffffull) - uint64_t(1), std::max(uint64_t(1), max - min)) + uint64_t(1)));
		}
		...
		constexpr int64_t next_int(int64_t min, int64_t max)
		{
			if (min == max)
				return min;
			return min + int64_t(next_uint() % (std::min(int64_t(9223372036854775807ll) - int64_t(1), std::max(int64_t(1), max - min)) + int64_t(1))); // we roll next_uint here to avoid negative value messing with range mapping
		}
		...
		constexpr int32_t next_int(int32_t min, int32_t max)
		{
			if (min == max)
				return min;
			return min + int32_t(next_uint() % (std::min(int32_t(2147483647) - int32_t(1), std::max(int32_t(1), max - min)) + int32_t(1))); // we roll next_uint here to avoid negative value messing with range mapping
		}
