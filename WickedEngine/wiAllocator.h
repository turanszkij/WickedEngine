...
namespace wi::allocator
{
	...
	struct weak_ptr
	{
		...
		shared_ptr<T> lock()
		{
			...
			SharedAllocator* alloc = get_allocator();
			T* ptr = get_ptr();
	...
	template<typename T, size_t block_size = 256>
	struct SharedBlockAllocator final : public SharedAllocator
	{
	...
	// Implementation of a thread-safe refcounted heap allocator
	template<typename T>
	...
	template<typename T, size_t block_size = 256>
	inline static SharedBlockAllocator<T, block_size>* shared_block_allocator = new SharedBlockAllocator<T, block_size>; // only destroyed after program exit, never earlier
	...
	// Create a new shared individually allocated object:
	...
	struct PageAllocator
	{
		...
		wi::allocator::shared_ptr<AllocatorInternal> allocator; // shared ptr is used to let any allocations extend the lifeftime of the allocator
		...
		void init(uint64_t total_size_in_bytes, uint32_t page_size = 64u * 1024u, bool deferred_release = false)
		{
			...
			allocator = wi::allocator::make_shared_single<AllocatorInternal>();
		...
		struct Allocation
		{
			wi::allocator::shared_ptr<AllocatorInternal> allocator; // the allocator is retained so that allocation can deallocate itself
