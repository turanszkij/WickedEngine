    ...
    void Update()
    {
        ...
        for(auto& event : events){
            switch(event.type){
                ...
                {
                    auto& controller = controllers.emplace_back();
                    controller.controller = SDL_OpenGamepad(event.gdevice.which);
                    if(controller.controller){
                        controller.portID = (Sint32)event.gdevice.which;
                        controller.internalID = SDL_GetGamepadID(controller.controller);
                    }
                    controller_map_rebuild();
                    break;
                }
