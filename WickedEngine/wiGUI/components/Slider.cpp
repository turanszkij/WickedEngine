...
namespace wi::gui
{
	void Slider::Create(
		...
		valueInputField.SetTooltip(
			"Enter number to modify value even outside slider limits. "
			"Arithmetic expressions are supported (e.g. 10 * 2, 1 + 0.5). "
			"Other inputs:\n"
		...
		valueInputField.OnInputAccepted(
			[this, start, end, defaultValue](EventArgs args)
		{
			if (args.sValue == "reset")
			{
				this->value = defaultValue;
				this->start = start;
				this->end   = end;
	...
	void Slider::Update(const wi::Canvas& canvas, const float dt)
	{
		...
		if (state != ACTIVE)
		{
			// Only set input field size when slider is not dragged because now
			// it will modify slider active size too, and slider size shouldn't
			// be modified while dragging!
			valueInputField.SetSize(XMFLOAT2(std::max(scale.y, wi::font::TextWidth(
				valueInputField.GetCurrentInputValue(),
				valueInputField.font.params
			) + 4), scale.y));

			valueInputField.SetPos(XMFLOAT2(
				translation.x + scale.x - valueInputField.GetSize().x,
				translation.y
			));
		}
		...
		{
			sprite_knob.params.siz.x = 16.0f;
		}
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (state == ACTIVE)
			{
				if (wi::input::Down(wi::input::MOUSE_BUTTON_LEFT)
				  || wi::input::IsTouchPanning()
					...
					if (state == ACTIVE)
					{
						// Continue drag if already grabbed whether it is
						// intersecting or not.
						dragged = true;
					}
				}
			...
			if (dragged)
			{
				...
				value = wi::math::InverseLerp(
					translation.x,
					translation.x + hitBox.siz.x,
					args.clickPos.x
				);
				...
				args.fValue = value;
				args.iValue = static_cast<int>(value);
		...
		sprites_knob[state].params.pos.x = wi::math::Lerp(
			translation.x + knobWidthHalf + 2,
			translation.x + hitBox.siz.x - knobWidthHalf - 2,
			wi::math::Clamp(wi::math::InverseLerp(
				start, end, value), 0, 1
					)
		);
	...
	void Slider::SetTheme(const Theme& theme, const int id)
	{
		...
		if (id > WIDGET_ID_SLIDER_BEGIN && id < WIDGET_ID_SLIDER_END)
		{
			...
			{
				theme.image.Apply(
					sprites_knob[id - WIDGET_ID_SLIDER_KNOB_IDLE].params
				);
			}
			else if (id >= WIDGET_ID_SLIDER_BASE_IDLE)
			{
				theme.image.Apply(
					sprites[id - WIDGET_ID_SLIDER_BASE_IDLE].params
				);
			}
		}
	}
