...
namespace wi::gui
{
	...
	{
		// If item doesn't exist or last then no children.
		if (items.size() <= static_cast<size_t>(index + 1))
		{
			return false;
		}

		// If item after index is exactly one level down, then it is its child.
		if (items[index].level + 1 == items[index + 1].level)
		{
			return true;
		}
	...
	{
		Hitbox2D retval = Hitbox2D(
			XMFLOAT2(translation.x, translation.y + item_height() + 1),
			XMFLOAT2(scale.x, scale.y - item_height() - 1)
		);
	...
	Hitbox2D TreeList::GetHitbox_Item(
		const int visible_count,
		const int level
	) const noexcept
	{
		const XMFLOAT2 pos = XMFLOAT2(
			translation.x + 2 + level * item_height(),
			translation.y + GetItemOffset(visible_count) + item_height() * 0.5f
		);
	...
	Hitbox2D TreeList::GetHitbox_ItemOpener(
		const int visible_count,
		const int level
	) const noexcept
	{
		...
		Hitbox2D hb = Hitbox2D(
			XMFLOAT2(pos.x, pos.y - item_height() * 0.25f),
			XMFLOAT2(item_height() * 0.5f, item_height() * 0.5f)
		);
	...
	{
		return scale.y < static_cast<int>(items.size()) * item_height();
	}
	...
	void TreeList::Update(const wi::Canvas& canvas, const float dt)
	{
		...
		if (IsEnabled())
		{
			...
			const Hitbox2D bottomhitbox = Hitbox2D(
			...
			if (wi::input::Down(wi::input::MOUSE_BUTTON_LEFT) || wi::input::IsTouchPanning())
			{
				if (resize_state != RESIZE_STATE_NONE)
				{
					...
					this->scale_local = wi::math::Max(
						this->scale_local,
						XMFLOAT3(item_height() * 3, item_height() * 3, 1)
		...
		active_area.pos.x = static_cast<float>(scissorRect.left);
		active_area.pos.y = static_cast<float>(scissorRect.top);
		active_area.siz.x = static_cast<float>(scissorRect.right) - static_cast<float>(scissorRect.left);
		active_area.siz.y = static_cast<float>(scissorRect.bottom) - static_cast<float>(scissorRect.top);
		...
		if (IsEnabled() && dt > 0)
		{
			...
			const Hitbox2D hitbox = Hitbox2D(
			...
			if (wheel_delta != 0.0f
				&& scroll_allowed
				&& scrollbar.IsScrollbarRequired()
				&& pointerHitbox.intersects(hitBox)
			) {
				const bool at_begin = (
					wheel_delta > 0 && scrollbar.IsScrolledToBegin()
				);
			...
			if (onReorder && dragging && !click_down)
			{
				if (drag_source >= 0)
				{
					...
					{
						// Dropped outside the widget -> go one level up in
						// hierarchy.
						EventArgs args;
					...
					else if (drag_reparent_target < 0
						&& drag_target >= 0
						...
						args.fValue   = static_cast<float>(drag_target_level);
						args.userdata = items[drag_source].userdata;
						args.bValue   = false;
			...
			if (scrollbar.GetState() == IDLE)
			{
				...
				for (Item& item : items)
				{
					...
					const Hitbox2D open_box = GetHitbox_ItemOpener(
						visible_count, item.level
					);
					...
					{
						// Item name box:
						const Hitbox2D name_box = GetHitbox_Item(
							visible_count,
							item.level
						);
						...
						if (name_box.intersects(pointerHitbox))
						{
							...
							if (clicked)
							{
								// Start potential drag (activated on movement
								// threshold).
								if (onReorder)
								{
									drag_source = i;
								...
								else
								{
									...
									else if (wi::input::Down(wi::input::KEYBOARD_BUTTON_LSHIFT) || wi::input::Down(wi::input::KEYBOARD_BUTTON_RSHIFT))
									{
										if (first_selected >= 0)
										{
											// shift: select range from first
											// selected up to current:
									...
									{
										// No control, no shift then only this
										// will be selected.
										ClearSelection();
									}
									Select(i);
			...
			// Compute drag_target, drag_reparent_target, and drag_indicator_y
			// while dragging.
			if (onReorder && dragging && click_down)
			{
				item_highlight       = drag_source;
				drag_target          = static_cast<int>(items.size()); // default: insert after all items
				drag_reparent_target = -1;
				drag_target_level    = 0;
				drag_indicator_y     = 0;
				...
				auto compute_drag_target_level = [&](int max_level) {
					...
					const float level_delta
						= (pointerHitbox.pos.x - drag_start_pos.x)
						/ item_height();

					const int level = source_level
						+ static_cast<int>(std::round(level_delta));
				...
				for (const Item& item : items)
				{
					...
					if (pointerHitbox.pos.y < item_bot_y - item_height() * 0.25f)
					{
						// Middle 50%: drop ON this item (re-parent).
						if (di != drag_source)
						{
							drag_reparent_target = di;
							drag_target          = static_cast<int>(items.size());
					...
					if (pointerHitbox.pos.y < item_bot_y)
					{
				...
				{
					drag_target_level = compute_drag_target_level(
						last_visible_level
					);
					drag_indicator_y = translation.y
					                 + GetItemOffset(last_visible)
									 + item_height();
				}

				// Clamp indicator line to list area (only when not
				// re-parenting).
				if (drag_reparent_target < 0)
				{
	...
	void TreeList::Render(const wi::Canvas& canvas, const CommandList cmd) const
	{
		...
		// resize indicator:
		{
			...
			fx.color = resize_state == RESIZE_STATE_NONE
			         ? sprites[FOCUS].params.color
					 : sprites[ACTIVE].params.color;
			...
			if (resize_state == RESIZE_STATE_BOTTOM
				|| pointerHitbox.intersects(bottomhitbox)
			) {
		...
		rect_without_scrollbar.left   = static_cast<int>(itemlist_box.pos.x);
		rect_without_scrollbar.right  = static_cast<int>(itemlist_box.pos.x + itemlist_box.siz.x);
		rect_without_scrollbar.top    = static_cast<int>(itemlist_box.pos.y);
		rect_without_scrollbar.bottom = static_cast<int>(itemlist_box.pos.y + itemlist_box.siz.y);
		...
		for (const Item& item : items)
		{
			...
			if (item.selected || item_highlight == i)
			{
				wi::image::Draw(nullptr, wi::image::Params(
					...
					name_box.siz.x,
					name_box.siz.y,
					sprites[item.selected ? FOCUS : IDLE].params.color),
				cmd);
			}
		...
		// Drop indicator line for drag-and-drop reorder (not shown when
		// re-parenting).
