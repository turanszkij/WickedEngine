...
void wiEmittedParticle::UpdateGPU(const TransformComponent& transform, const MaterialComponent& material, const MeshComponent* mesh, CommandList cmd) const
{
	...
	if (!IsPaused())
	{
		...
		GPUBarrier buffer0 = GPUBarrier::Buffer(&indirectBuffers, BUFFER_STATE_INDIRECT_ARGUMENT, BUFFER_STATE_UNORDERED_ACCESS);
		...
		GPUBarrier memory0 = GPUBarrier::Memory();
		device->Barrier(&memory0, 1, cmd);
		device->EventEnd(cmd);

		GPUBarrier buffer1 = GPUBarrier::Buffer(&indirectBuffers, BUFFER_STATE_UNORDERED_ACCESS, BUFFER_STATE_INDIRECT_ARGUMENT);
		...
		GPUBarrier memory1 = GPUBarrier::Memory();
		device->Barrier(&memory1, 1, cmd);
		device->EventEnd(cmd);
		...
		if (IsSPHEnabled())
		{
			...
			GPUBarrier memory2 = GPUBarrier::Memory();
			device->Barrier(&memory2, 1, cmd);
			device->EventEnd(cmd);
			...
			GPUBarrier memory4 = GPUBarrier::Memory();
			device->Barrier(&memory4, 1, cmd);
			device->EventEnd(cmd);
			...
			device->DispatchIndirect(&indirectBuffers, ARGUMENTBUFFER_OFFSET_DISPATCHSIMULATION, cmd);
			GPUBarrier memory5 = GPUBarrier::Memory();
			device->Barrier(&memory5, 1, cmd);
			device->EventEnd(cmd);
			...
			device->DispatchIndirect(&indirectBuffers, ARGUMENTBUFFER_OFFSET_DISPATCHSIMULATION, cmd);
			GPUBarrier memory6 = GPUBarrier::Memory();
			device->Barrier(&memory6, 1, cmd);
			device->EventEnd(cmd);
			...
			device->DispatchIndirect(&indirectBuffers, ARGUMENTBUFFER_OFFSET_DISPATCHSIMULATION, cmd);
			GPUBarrier memory7 = GPUBarrier::Memory();
			device->Barrier(&memory7, 1, cmd);
			device->EventEnd(cmd);
		...
		device->DispatchIndirect(&indirectBuffers, ARGUMENTBUFFER_OFFSET_DISPATCHSIMULATION, cmd);
		GPUBarrier memory8 = GPUBarrier::Memory();
		device->Barrier(&memory8, 1, cmd);
		device->EventEnd(cmd);
	...
	if (!IsPaused())
	{
		...
		device->Dispatch(1, 1, 1, cmd);
		GPUBarrier memory9 = GPUBarrier::Memory();
		device->Barrier(&memory9, 1, cmd);
