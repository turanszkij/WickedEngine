...
namespace wi
{
	...
	inline XMVECTOR __vectorcall world_to_uvw(XMVECTOR P, XMVECTOR center, XMVECTOR resolution_rcp, XMVECTOR voxelSize_rcp)
	{
		XMVECTOR diff = (P - center) * resolution_rcp * voxelSize_rcp;
		XMVECTOR uvw = diff * XMVectorSet(0.5f, -0.5f, 0.5f, 1) + XMVectorReplicate(0.5f);
		return uvw;
	}
	inline XMVECTOR __vectorcall uvw_to_world(XMVECTOR uvw, XMVECTOR center, XMVECTOR resolution, XMVECTOR voxelSize)
	{
