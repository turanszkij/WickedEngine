...
void main(uint3 DTid : SV_DispatchThreadID, uint3 Gid : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
	...
	particleBuffer[instanceID].tangent_random = tangent_random;
	...
	particleBuffer[instanceID].binormal_length = binormal_length;

	[branch]
	if (xHairRegenerate)
	{
		particleBuffer[instanceID].normal = target;
	}
	...
	// Store particle:
	particleBuffer[instanceID].position = position;
