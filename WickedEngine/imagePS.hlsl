...
float4 main(VertextoPixel PSIn) : SV_TARGET
{
	...
	[branch]if(xMaskFadOpaDis.w==1){
		...
		float2 distort=normalize( float3(xNormalMap.SampleLevel(Sampler,PSIn.tex,0).rg,1) )*0.04f*(1-xMaskFadOpaDis.z);
		PSIn.tex.xy=distortionCo+distort;
	}

	color += xTexture.SampleLevel(Sampler,PSIn.tex+PSIn.tex.zw,0);
	[branch]if(xMaskFadOpaDis.x==1)
		color *= xMaskTex.SampleLevel(Sampler,PSIn.tex,0).a;
