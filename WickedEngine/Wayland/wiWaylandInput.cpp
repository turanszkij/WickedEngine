...
namespace wi::input::wayland {
	...
	/**
	 * Code imported and adapted from SDL2
	 * https://github.com/libsdl-org/SDL/blob/60c65f9fa428cbde2c1cc60debb3f8bd4a92f742/src/events/SDL_keysym_to_scancode.c#L392
	 * @param keysym
	 * @param keycode
	 * @return wicked code
	 */
	int64_t wiSDL_GetScancodeFromKeySym(Uint32 keysym, Uint32 keycode);
	int64_t to_wicked(uint32_t key, uint32_t keyk);
	int64_t to_wicked2(uint32_t key, uint32_t keyk);

    bool use_xkb_v1_keymap = false;
	struct xkb_context *xkb_context = nullptr;
	struct xkb_keymap *xkb_keymap = nullptr;
	struct xkb_state *xkb_keyboard_state = nullptr;

    void Initialize() {
    	xkb_context = xkb_context_new(XKB_CONTEXT_NO_FLAGS);
    	xkb_keymap = nullptr;
    	if (!xkb_context)
    		wi::backlog::post("[Wayland Input] XKB common failed to initialize, keyboard input will not work correctly.");
    }

	void Deinit() {
		using namespace wi::wayland;
		safe_delete(xkb_keyboard_state, xkb_state_unref);
		safe_delete(xkb_keymap, xkb_keymap_unref);
		safe_delete(xkb_context, xkb_context_unref);
    }

    void SetKeyMap(uint32_t format, int32_t fd, uint32_t size) {
    	if (!xkb_context) return;
        assert(format == WL_KEYBOARD_KEYMAP_FORMAT_NO_KEYMAP || format == WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1);
        use_xkb_v1_keymap = (format == WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1);

    	if (use_xkb_v1_keymap) {
    		char* shared_memory = static_cast<char *>(mmap(nullptr, size, PROT_READ, MAP_PRIVATE, fd, 0));

    		xkb_keymap = xkb_keymap_new_from_string(
					xkb_context, shared_memory, XKB_KEYMAP_FORMAT_TEXT_V1,
					XKB_KEYMAP_COMPILE_NO_FLAGS);
    		if (!xkb_keymap) {
    			wi::backlog::post("[Wayland Input] XKB keymap failed to initialize, keyboard input will not work correctly.");
    		} else {
    			xkb_keyboard_state = xkb_state_new(xkb_keymap);
    			if (!xkb_keyboard_state)
    				wi::backlog::post("[Wayland Input] XKB keyboard state failed to initialize, keyboard input will not work correctly.");
    		}
    ...
    void ProcessKeyboardEvent(const uint32_t time, const uint32_t key, const uint32_t state) {
    	wl_keyboard_key_state keystate = (wl_keyboard_key_state) state;
        const bool pressed = (keystate == WL_KEYBOARD_KEY_STATE_PRESSED);
    	uint32_t keyscan = key;
    	uint32_t keycode = key;
        if (use_xkb_v1_keymap && xkb_keyboard_state) {
        	const xkb_keysym_t keysym = xkb_state_key_get_one_sym(xkb_keyboard_state, key + 8);
	        //TODO do we want to convert from scancodes to characters actually? we might want to use raw scancode in a
	        //game engine instead! Providing both codes could actually prove the best.
        	keycode = keysym;

        	// const xkb_keysym_t *keysyms = nullptr;
        	// int num_keys = xkb_state_key_get_syms(xkb_keyboard_state, key + 8, &keysyms);
        	// if (num_keys > 0) {
        	// 	keyscan = keysyms[0];
        	// }
        }

        int64_t wicked_key = wiSDL_GetScancodeFromKeySym(keyscan, key);
    	fprintf(stderr, "Key is %d (%d, %d:%c) -> (%ld:%c) state is %s\n", key, keyscan, keycode, keycode, wicked_key, (int)wicked_key, pressed ? "pressed" : "released");
		wicked_key = to_wicked2(keycode, keyscan);
    	if (wicked_key > 0)
			keyboard.buttons[wicked_key] = pressed;
    }

    void ProcessPointerButton(uint32_t time, uint32_t button, uint32_t state) {
        const bool pressed = static_cast<bool>(state);
        switch (button) {
            case BTN_LEFT: mouse.left_button_press = pressed; break;
    ...
    void ProcessPointerMotion(uint32_t time, double x, double y) {
        XMFLOAT2 old_pos = mouse.position;
        mouse.position.x = (float) x / 1.0f;
        mouse.position.y = (float) y / 1.0f;
        mouse.delta_position.x += mouse.position.x - old_pos.x;
        mouse.delta_position.y += mouse.position.y - old_pos.y;
    }
	...
	int64_t to_wicked(const uint32_t key, const uint32_t keyk) {
    	if (key >= '0' && key <= '9')
    		return (key - '0') + DIGIT_RANGE_START;
    	if (key >= 'A' && key <= 'Z')
    		return (key - 'A') + CHARACTER_RANGE_START;
    	if (key >= 'a' && key <= 'z')
    		return (key - 'a') + CHARACTER_RANGE_START;
    	//TODO continue
...
static const struct {
	...
	{1, ''}, // ESC
	{41, ''}, // `
	{2, DIGIT_RANGE_START+1}, // 1
	{3, DIGIT_RANGE_START+2}, // 2
	{4, DIGIT_RANGE_START+3}, // 3
	{5, DIGIT_RANGE_START+4}, // 4
	{6, DIGIT_RANGE_START+5}, // 5
	{7, DIGIT_RANGE_START+6}, // 6
	{8, DIGIT_RANGE_START+7}, // 6
	{9, DIGIT_RANGE_START+8}, // 6
	{10, DIGIT_RANGE_START+9}, // 6
	{11, DIGIT_RANGE_START}, // 0
	{12, '-'}, // -
	{13, '='}, // =
};

	int64_t to_wicked2(const uint32_t keycode, const uint32_t keyscan) {
		for (const auto&[e_keyscan, e_wikey] : KeySymToWiScancode) {
			if (e_keyscan == keyscan) return e_wikey;
		}
        return -1;
    }

// ----------------------------------------------------------------------------
// Code section imported and adapted from SDL2 --------------------------------
...
/* *INDENT-ON* */ // clang-format on
	...
	int64_t wiSDL_GetScancodeFromKeySym(Uint32 keysym, Uint32 keycode)
    {
    	...
    	for (i = 0; i < std::size(KeySymToSDLScancode); ++i) {
    	...
    	if (!linux_keycode) {
    		...
    		i = keycode;
    		if (i >= 0 && i < std::size(LinuxKeycodeKeysyms) && keysym == LinuxKeycodeKeysyms[i]) {
    			linux_keycode = i;
    		} else {
    			// Scan the table for this keysym
    			for (i = 0; i < std::size(LinuxKeycodeKeysyms); ++i) {
    	...
    	if (!linux_keycode) {
    		...
    		for (i = 0; i < std::size(ExtendedLinuxKeycodeKeysyms); ++i) {
    	...
    	return -1;//SDL_GetScancodeFromTable(SDL_SCANCODE_TABLE_LINUX, linux_keycode);
    }

// END Code section imported and adapted from SDL2 ----------------------------
//-----------------------------------------------------------------------------
