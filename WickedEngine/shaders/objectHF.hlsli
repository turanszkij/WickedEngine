...
inline ShaderTerrainChunk GetTerrainChunk(float3 P, out float2 chunk_min, out float2 chunk_max)
{
	...
	if(terrain.chunk_buffer >= 0)
	{
		...
		{
			chunk_min = terrain.center_chunk_pos.xz + chunk_coord * terrain.chunk_size;
			chunk_max = terrain.center_chunk_pos.xz + terrain.chunk_size + chunk_coord * terrain.chunk_size;
			uint chunk_idx = flatten2D(chunk_coord + terrain.chunk_buffer_range, terrain.chunk_buffer_range * 2 + 1);
			return bindless_structured_terrain_chunks[terrain.chunk_buffer][chunk_idx];
		}
	}
	ShaderTerrainChunk ret;
	ret.init();
	chunk_min = 0;
	chunk_max = 0;
	return ret;
...
// Pixel shader base:
{
	...
	if (GetMaterial().blend_with_terrain_height_rcp > 0)
	{
		...
		float2 chunk_min, chunk_max;
		ShaderTerrainChunk chunk = GetTerrainChunk(surface.P, chunk_min, chunk_max);
		[branch]
