...
struct Variance
{
	...
	float update(half4 current)
	{
		...
		half4 mean = sum / half(arraysize(history) + 1);

		half4 variance = sqr(current - mean);
		for (uint i = 0; i < arraysize(history); ++i)
		{
			variance += sqr(history[i] - mean);
		}
		variance /= (half)arraysize(history);
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	for (uint i = 0; i < unroll_count; ++i)
	{
		...
		int2 halfResCoord = DTid.xy * 2 + halfResIndexToCoordinateOffset[subPixelIndex];
		...
		variance_amount = 1 - sqr(1 - variance_amount);
