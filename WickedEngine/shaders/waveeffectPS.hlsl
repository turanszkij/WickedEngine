...
float4 main(in float4 pos : SV_Position, in float2 uv_screen : TEXCOORD) : SV_Target
{
	...
	float2 uv = 2.0 * uv_screen - 1.0 - sin(uv_screen.x * 5 + iTime * 1.8) * 0.34;

	float waves = 0;
	for (uint i = 0; i < 6; i++)
	{
		uv.y += (0.2 * sin(uv.x * 4 + i * 2 - iTime * 0.1));
		float li = 0.025 + smoothstep(0.0, 0.8, pow(abs(fmod(uv_screen.x + i / 1.1 + iTime, 2.0) - 1.0), 2.0));
		float gw = abs(li / (64.0 * uv.y));
