...
PUSHCONSTANT(capture, SkyAtmosphereCapturePushConstants);
#else
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	Render(DTid, uv, depth, depthWorldPosition, rayOrigin, rayDirection, luminance, transmittance, true);
...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	float3 luminance = float3(0.0, 0.0, 0.0);
	float transmittance = 0.0;
	Render(DTid, uv, depth, depthWorldPosition, rayOrigin, rayDirection, luminance, transmittance, false);
