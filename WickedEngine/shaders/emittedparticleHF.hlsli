...
{
	return load_geometry(EmitterGetInstance().geometryOffset, 0).mesh;
}
ShaderMaterial EmitterGetMaterial()
{
	return load_material(load_geometry(EmitterGetInstance().geometryOffset, 0).subset.materialIndex);
}
