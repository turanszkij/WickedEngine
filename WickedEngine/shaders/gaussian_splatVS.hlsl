...
half3 fetchViewDependentRadiance(in uint splatIndex, in float3 worldViewDir)
{
	...
	half3 shd1[3] = {
		unpack_half3(splats[splatIndex].f_rest[0]),
		unpack_half3(splats[splatIndex].f_rest[1]),
		unpack_half3(splats[splatIndex].f_rest[2]),
	};
	// SH coefficients for degree 2 (4 5 6 7 8)
	half3 shd2[5] = {
		unpack_half3(splats[splatIndex].f_rest[3]),
		unpack_half3(splats[splatIndex].f_rest[4]),
		unpack_half3(splats[splatIndex].f_rest[5]),
		unpack_half3(splats[splatIndex].f_rest[6]),
		unpack_half3(splats[splatIndex].f_rest[7]),
	};
	// SH coefficients for degree 3 (9,10,11,12,13,14,15)
	half3 shd3[7] = {
		unpack_half3(splats[splatIndex].f_rest[8]),
		unpack_half3(splats[splatIndex].f_rest[9]),
		unpack_half3(splats[splatIndex].f_rest[10]),
		unpack_half3(splats[splatIndex].f_rest[11]),
		unpack_half3(splats[splatIndex].f_rest[12]),
		unpack_half3(splats[splatIndex].f_rest[13]),
		unpack_half3(splats[splatIndex].f_rest[14]),
	};
...
void main(in uint vertexID : SV_VertexID, in uint instanceID : SV_InstanceID, out float4 pos : SV_Position, out half4 color : COLOR, out half2 localPos : LOCALPOS)
{
	...
	if (!threedgsProjectedExtentBasis(cov2Dv, sqrt8, /*frameInfo.splatScale*/1, /*splatColor.a*/color.a, basisVector1, basisVector2))
	{
		// emit same vertex to get degenerate triangle
		pos = float4(0.0, 0.0, 2.0, 1.0);
	}
	else
	{
