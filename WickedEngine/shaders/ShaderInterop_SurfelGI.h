...
static const float SURFEL_RECYCLE_PRESSURE_FLOOR = 0.02f; // under-target trickle: per-frame eviction scale for fully-stale surfels (recency 1); recency-gated so a surfel seen this frame is never recycled while under target
