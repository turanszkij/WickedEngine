...
static const float SURFEL_COVERAGE_TEMPORAL_BLEND = 0.2f; // fraction of THIS frame's freshly gathered GI blended into the temporal history each frame in surfel_coverage (the rest is carried from history). Lower = more temporal smoothing/denoising and more history reuse, but slower to react to lighting changes; 1 = no accumulation (history ignored). Stage 1 blends the SAME screen pixel (no reprojection), so a static view denoises but camera motion ghosts until reprojection is added
