...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	const float depth = texture_depth[pixel * VXGI_DIFFUSE_UPSAMPLING];

	float4 color = 0;
	[branch]
	if (depth > 0)
	{
		const float roughness = texture_roughness[pixel * VXGI_DIFFUSE_UPSAMPLING];
		const float3 N = decode_oct(texture_normal[pixel * VXGI_DIFFUSE_UPSAMPLING]);
...
// This was the version with temporal upscaler:
