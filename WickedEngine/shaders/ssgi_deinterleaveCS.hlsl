...
void main(uint3 Gid : SV_GroupID, uint groupIndex : SV_GroupIndex, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    shared_normals[destIdx + 0] = pack_half2(texture_normal_roughness[min(startST | uint2(0, 0), dim - 1)] * 2 - 1);
    shared_normals[destIdx + 8] = pack_half2(texture_normal_roughness[min(startST | uint2(8, 0), dim - 1)] * 2 - 1);
    shared_normals[destIdx + 128] = pack_half2(texture_normal_roughness[min(startST | uint2(0, 8), dim - 1)] * 2 - 1);
    shared_normals[destIdx + 136] = pack_half2(texture_normal_roughness[min(startST | uint2(8, 8), dim - 1)] * 2 - 1);
