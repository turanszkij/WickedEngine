...
private:
	...
	wiContainer::vector<uint8_t> DATA; // data suitable for read/write operations
...
public:
	...
	template<typename T>
	inline wiArchive& operator<<(const wiContainer::vector<T>& data)
	{
	...
	template<typename T>
	inline wiArchive& operator>>(wiContainer::vector<T>& data)
	{
		// Here we will use the >> operator so that non-specified types will have compile error!
		size_t count;
		(*this) >> count;
