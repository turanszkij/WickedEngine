...
namespace wiHelper
{
	...
	bool saveTextureToMemory(const wiGraphics::Texture& texture, wiContainer::vector<uint8_t>& texturedata)
	{
	...
	bool saveTextureToMemoryFile(const wiGraphics::Texture& texture, const std::string& fileExtension, wiContainer::vector<uint8_t>& filedata)
	{
		using namespace wiGraphics;
		TextureDesc desc = texture.GetDesc();
		wiContainer::vector<uint8_t> texturedata;
	...
	bool saveTextureToMemoryFile(const wiContainer::vector<uint8_t>& texturedata, const wiGraphics::TextureDesc& desc, const std::string& fileExtension, wiContainer::vector<uint8_t>& filedata)
	{
		using namespace wiGraphics;
		uint32_t data_count = desc.width * desc.height;
		...
		stbi_write_func* func = [](void* context, void* data, int size) {
			wiContainer::vector<uint8_t>& filedata = *(wiContainer::vector<uint8_t>*)context;
	...
	bool saveTextureToFile(const wiGraphics::Texture& texture, const std::string& fileName)
	{
		using namespace wiGraphics;
		TextureDesc desc = texture.GetDesc();
		wiContainer::vector<uint8_t> data;
	...
	bool saveTextureToFile(const wiContainer::vector<uint8_t>& texturedata, const wiGraphics::TextureDesc& desc, const std::string& fileName)
	{
		using namespace wiGraphics;
	...
	bool FileRead(const std::string& fileName, wiContainer::vector<uint8_t>& data)
	{
#ifndef PLATFORM_UWP
	...
	void FileDialog(const FileDialogParams& params, std::function<void(std::string fileName)> onSuccess)
	{
		...
		std::thread([=] {
			...
			wiContainer::vector<char> filter;
			filter.reserve(256);
			{
		...
		wiContainer::vector<std::string> extensions = {params.description, ""};
		for (auto& x : params.extensions)
		{
			extensions[1] += "*." + x + " ";
		}
		...
		switch (params.type) {
			case FileDialogParams::OPEN: {
				wiContainer::vector<std::string> selection = pfd::open_file(
					"Open file",
