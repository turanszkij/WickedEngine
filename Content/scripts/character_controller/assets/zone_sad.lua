    ...
    for i,entity in ipairs(scene.Entity_GetExpressionArray()) do
        ...
        if expression ~= nil and transform ~= nil then
            ...
            if vector.Subtract(expression_pos, zone_pos).Length() < distance then
                expression.SetPresetWeight(ExpressionPreset.Sad, 1)
                zone_active = true
            else
                expression.SetPresetWeight(ExpressionPreset.Sad, 0)
            end
        end
    end

    local str = "If character gets close to this, it will be sad\n"
    if zone_active then
        str = str .. "...and a character is currently sad!"
    else
        str = str .. "...but no one is sad currently."
    end
