    ...
    for i,entity in ipairs(scene.Entity_GetExpressionArray()) do
        ...
        if expression ~= nil and transform ~= nil then
            ...
            if vector.Subtract(expression_pos, zone_pos).Length() < distance then
                expression.SetPresetWeight(ExpressionPreset.Angry, 1)
                zone_active = true
            else
                expression.SetPresetWeight(ExpressionPreset.Angry, 0)
            end
        end
    end

    local str = "If character gets close to this, it will be angry\n"
    if zone_active then
        str = str .. "...and a character is currently angry!"
    else
        str = str .. "...but no one is angry currently."
    end
