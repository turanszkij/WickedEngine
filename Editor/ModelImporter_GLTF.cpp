	...
	if (env != state.gltfModel.extensions.end())
	{
		...
		for (int i = 0; i < (int)lights.ArrayLen(); ++i)
		{
			...
			if (light.Has("specularImages"))
			{
				...
				wi::vector<wi::vector<XMFLOAT3PK>> hdr_datas(mip_count * 6);
				...
				for (int m = 0; m < mip_count; ++m)
				{
					...
					for (int f = 0; f < face_count; ++f)
					{
						...
						wi::vector<XMFLOAT3PK>& hdr_data = hdr_datas[idx];
						...
						for (int y = 0; y < height; ++y)
						{
							for (int x = 0; x < width; ++x)
							{
								...
								hdr_data[x + y * width] = XMFLOAT3PK(unpk.x, unpk.y, unpk.z);
							}
						}
...
#define MAKE_FOURCC(a, b, c, d) (((uint32_t)(d) << 24) | ((uint32_t)(c) << 16) | ((uint32_t)(b) << 8) | (uint32_t)(a))
				...
				static constexpr DWORD magic = MAKE_FOURCC('D', 'D', 'S', ' ');

				DDS_HEADER header = {};
				header.dwSize = sizeof(DDS_HEADER);
				header.dwFlags = DDSD_CAPS | DDSD_WIDTH | DDSD_HEIGHT | DDSD_PIXELFORMAT | DDSD_MIPMAPCOUNT;
				header.dwWidth = desc.width;
				header.dwHeight = desc.height;
				header.dwPitchOrLinearSize = desc.width * sizeof(XMFLOAT3PK);
				header.dwDepth = 0;
				header.dwMipMapCount = desc.mip_levels;
				...
				header10.dxgiFormat = DXGI_FORMAT_R11G11B10_FLOAT;
				...
				header10.arraySize = 1; // number of cubes or for 3D textures it is always 1

				size_t wholeDataSize = 0;
				for (auto& x : hdr_datas)
				{
					wholeDataSize += x.size() * sizeof(XMFLOAT3PK);
				}
				...
				uint8_t* dst = dds.data();
				std::memcpy(dst, &magic, sizeof(magic));
				dst += sizeof(magic);
				std::memcpy(dst, &header, sizeof(header));
				dst += sizeof(header);
				std::memcpy(dst, &header10, sizeof(header10));
				dst += sizeof(header10);
				for (auto& x : hdr_datas)
				{
					std::memcpy(dst, x.data(), x.size() * sizeof(XMFLOAT3PK));
					dst += x.size() * sizeof(XMFLOAT3PK);
				}

				weather.skyMapName = "GLTFEnvironmentMap.dds";
