...
void PaintToolWindow::Create(EditorComponent* editor)
{
	...
	Translate(XMFLOAT3((float)editor->main->canvas.GetLogicalWidth() - 550, 50, 0));
	SetVisible(false);
}
...
void PaintToolWindow::Update(float dt)
{
	...
	XMFLOAT2 posNew;
	posNew.x = editor->main->canvas.GetLogicalWidth() * 0.5f;
	posNew.y = editor->main->canvas.GetLogicalHeight() * 0.5f;
	...
	const XMVECTOR SCREEN = XMVectorSet((float)editor->main->canvas.GetLogicalWidth(), (float)editor->main->canvas.GetLogicalHeight(), 1, 1);
